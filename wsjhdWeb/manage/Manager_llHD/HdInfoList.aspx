<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="HdInfoList.aspx.cs" Inherits="NewInfoWeb.manage.Manager_llHD.HdInfoList" %>
                    ...
                    <asp:Repeater ID="rptLoop" runat="server">
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td class="list_li_txt3" style="width: 10%" title="<%#Eval("Extend") %>">
                                    <%#Eval("Extend") %>
                                </td>
                                <td class="list_li_txt3" style="width: 24%">
                                    <a href="/webInfo/llhd/infoDetal.aspx?tid=<%#Eval("Id") %>" target="_blank" style="color: black;">
                                        查看详情</a> <a style="color: red;" href="HdInfoList.aspx?del=<%#Eval("Id") %>&key=<%#Request["key"] %>"
                                            onclick="return confirm('删除后将无法恢复，您确定要删除吗？');">删除</a><a href="HdPostList.aspx?tid=<%#Eval("Id") %>&page=<%#AspNetPager1.CurrentPageIndex %>&key=<%#Request["key"] %>"
                                                class="<%#Eval("Status").ToString()=="0"?"hd":"" %>">查看回复信息</a>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
