...
namespace NewInfoWeb.yqh
{
    ...
    /// helper1 的摘要说明
    /// </summary>
    public class helper1 : IHttpHandler
    {
        ...
        private void AddWSJInfo(HttpContext context)
        {
            try
            {
                ...
                var starttime = WebConfigurationManager.AppSettings["starttime"];
                var endtime = WebConfigurationManager.AppSettings["endtime"];
                if (context.Request.Cookies["HSGGWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["HSGGWebDomain"]["iswxh"]))
                {
                ...
                else
                {
                    ...
                    {
                        if (context.Request.Cookies["HSGGWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["HSGGWebDomain"]["useropid"]))
                        {
                        ...
                        else
                        {
                            ...
                            using (WXDBEntities db = new WXDBEntities())
                            {
                                ...
                                //    return;
                                //}
                                var modelt = db.Forms.Where(s => s.Name.Equals(tname)&&s.Type.Equals(8)).FirstOrDefault();
                                ...
                                {
                                    var tmpopid = context.Request.Cookies["HSGGWebDomain"]["useropid"];
                                    var model = db.Forms.Where(s => s.Source.Equals(tmpopid) && s.Type.Equals(8)).FirstOrDefault();
                                    var model1 = db.Forms.Where(s => s.Type.Equals(8) && s.Mobile.Equals(tphone)).FirstOrDefault();
                                    ...
                                    else
                                    {
                                        ...
                                        var total = db.Forms.Where(s => s.Type.Equals(8)).Select(s => s.JFCount).Sum();
                                        var totalnum = Convert.ToDouble(WebConfigurationManager.AppSettings["totalHB"]);
                                        if (total >= totalnum)
                                        {
        ...
        private string SendHB(HttpContext context, string opid, string name, string phone, string hb)
        {
            ...
            packageReqHandler.SetParameter("send_name", WebConfigurationManager.AppSettings["sendname"]);                 //红包发送者名称
