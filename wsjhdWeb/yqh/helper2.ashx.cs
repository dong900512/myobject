...
namespace NewInfoWeb.yqh
{
    ...
    /// helper2 的摘要说明
    /// </summary>
    public class helper2 : IHttpHandler
    {
        ...
        private void AddWSJInfo(HttpContext context)
        {
            try
            {
                ...
                var starttime = WebConfigurationManager.AppSettings["starttime1"];
                var endtime = WebConfigurationManager.AppSettings["endtime1"];
                if (context.Request.Cookies["HFSPWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["HFSPWebDomain"]["iswxh"]))
                {
                ...
                else
                {
                    ...
                    {
                        if (context.Request.Cookies["HFSPWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["HFSPWebDomain"]["useropid"]))
                        {
                        ...
                        else
                        {
                            ...
                            using (WXDBEntities db = new WXDBEntities())
                            {
                                ...
                                //    return;
                                //}
                                var modelt = db.Forms.Where(s => s.Name.Equals(tname) && s.Type.Equals(9)).FirstOrDefault();
                                ...
                                {
                                    var tmpopid = context.Request.Cookies["HFSPWebDomain"]["useropid"];
                                    var model = db.Forms.Where(s => s.Source.Equals(tmpopid) && s.Type.Equals(9)).FirstOrDefault();
                                    var model1 = db.Forms.Where(s => s.Type.Equals(9) && s.Mobile.Equals(tphone)).FirstOrDefault();
                                    string tmpphone = WebConfigurationManager.AppSettings["phonelist"];
                                    if (model != null || model1 != null || !(tmpphone.Contains(tphone)))
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        var total = db.Forms.Where(s => s.Type.Equals(9)).Select(s => s.JFCount).Sum();
                                        var totalnum = Convert.ToDouble(WebConfigurationManager.AppSettings["totalHB1"]);
                                        if (total >= totalnum)
                                        {
        ...
        private string SendHB(HttpContext context, string opid, string name, string phone, string hb)
        {
            ...
            packageReqHandler.SetParameter("send_name", WebConfigurationManager.AppSettings["sendname1"]);                 //红包发送者名称
            ...
            packageReqHandler.SetParameter("wishing", WebConfigurationManager.AppSettings["hbzf1"]);               //红包祝福语
            packageReqHandler.SetParameter("client_ip", context.Request.UserHostAddress);               //调用接口的机器Ip地址
            packageReqHandler.SetParameter("act_name", WebConfigurationManager.AppSettings["hbname1"]);   //活动名称
            packageReqHandler.SetParameter("remark", WebConfigurationManager.AppSettings["hbDesc1"]);   //备注信息
