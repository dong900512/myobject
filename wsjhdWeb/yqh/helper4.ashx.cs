...
namespace NewInfoWeb.yqh
{
    ...
    /// helper4 的摘要说明
    /// </summary>
    public class helper4 : IHttpHandler
    {
        ...
        private void AddWSJInfo(HttpContext context)
        {
            try
            {
                ...
                var starttime = WebConfigurationManager.AppSettings["starttime4"];
                var endtime = WebConfigurationManager.AppSettings["endtime4"];
                if (context.Request.Cookies["BRWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["BRWebDomain"]["iswxh"]))
                {
                ...
                else
                {
                    ...
                    {
                        if (context.Request.Cookies["BRWebDomain"] == null || string.IsNullOrEmpty(context.Request.Cookies["BRWebDomain"]["useropid"]))
                        {
                        ...
                        else
                        {
                            ...
                            using (WXDBEntities db = new WXDBEntities())
                            {
                                ...
                                //    return;
                                //}
                                var modelt = db.Forms.Where(s => s.Name.Equals(tname) && s.Type.Equals(11)).FirstOrDefault();
                                ...
                                {
                                    var tmpopid = context.Request.Cookies["BRWebDomain"]["useropid"];
                                    var model = db.Forms.Where(s => s.Source.Equals(tmpopid) && s.Type.Equals(11)).FirstOrDefault();
                                    var model1 = db.Forms.Where(s => s.Type.Equals(11) && s.Mobile.Equals(tphone)).FirstOrDefault();
                                    ...
                                    else
                                    {
                                        ...
                                        var total = db.Forms.Where(s => s.Type.Equals(11)).Select(s => s.JFCount).Sum();
                                        var totalnum = Convert.ToDouble(WebConfigurationManager.AppSettings["totalHB4"]);
                                        if (total >= totalnum)
                                        {
        ...
        private string SendHB(HttpContext context, string opid, string name, string phone, string hb)
        {
            ...
            packageReqHandler.SetParameter("send_name", WebConfigurationManager.AppSettings["sendname4"]);                 //红包发送者名称
            ...
            packageReqHandler.SetParameter("wishing", WebConfigurationManager.AppSettings["hbzf4"]);               //红包祝福语
            packageReqHandler.SetParameter("client_ip", context.Request.UserHostAddress);               //调用接口的机器Ip地址
            packageReqHandler.SetParameter("act_name", WebConfigurationManager.AppSettings["hbname4"]);   //活动名称
            packageReqHandler.SetParameter("remark", WebConfigurationManager.AppSettings["hbDesc4"]);   //备注信息
            ...
            try
            {
                ...
                if ("SUCCESS".Equals(return_code))
                {
                    ...
                    if ("SUCCESS".Equals(result_code))
                    {
                        ...
                        using (WXDBEntities db = new WXDBEntities())
                        {
                            var model = db.Forms.Where(s => s.Source.Equals(opid) && s.Type.Equals(11)).FirstOrDefault();
                            if (model != null)
                            {
                                str = "2|发送红包失败！";
                            }
