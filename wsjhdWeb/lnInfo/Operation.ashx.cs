    ...
    public class Operation : IHttpHandler
    {
        ...
        private void AddLnqpInfo()
        {
            try
            {
                ...
                var tpicurl = Dos.Common.CookieHelper.Get("curlnQPImgUrl");
                var tpnickname = Dos.Common.CookieHelper.Get("curlnQPName");
                var tmodel = DbSession.Default.From<Dos.Model.HdPic>().Where(s => s.Extend2.Equals("110") && s.Extend1.Equals(topid1)).ToFirstDefault();
                if (tmodel.Id > 0)
                {
                ...
                {
                    var tmo = DbSession.Default.From<Dos.Model.HdPic>().Where(t => t.Extend4.Equals(tphone) && t.Extend2.Equals("110")).ToFirstDefault();
                    if (tmo.Id > 0)
                    {
                    ...
                    {
                        var tid = DbSession.Default.From<Dos.Model.HdPic>().Where(s => s.Extend2.Equals("110")).Count();
                        //tmodel.AttachAll()
