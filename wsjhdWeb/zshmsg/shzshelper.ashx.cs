...
namespace NewInfoWeb.zshmsg
{
    ...
    /// shzshelper 的摘要说明
    /// </summary>
    public class shzshelper : IHttpHandler
    {
        ...
        protected void GetListInfo()
        {
            using (WXDBEntities db = new WXDBEntities())
            {
                ...
                list = db.lotterUserInfo.Where(s => s.Type.Equals(14) && s.PriceNumber != 0).ToList();
        ...
        protected void YjInfo()
        {
            ...
            try
            {
                ...
                if (DateTime.Compare(t1, t2) > 0 && DateTime.Compare(t1, t3) < 0)
                {
                    using (WXDBEntities db = new WXDBEntities())
                    {
                        var countnum = db.lotterUserInfo.Where(s => s.Type.Equals(14)).Count();

                        var mod1 = db.lotterUserInfo.Where(s => s.Type.Equals(14) && s.OpenId.Equals(stropid)).FirstOrDefault();
                        if (mod1 != null)
                        {
